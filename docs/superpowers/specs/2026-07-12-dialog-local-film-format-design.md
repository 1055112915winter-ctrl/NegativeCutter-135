...
- Auto starts with frame count `0`, which enables both automatic frame-count detection and automatic 135/120 geometry routing while passing no explicit format hint.
- Selecting a format in the current dialog immediately replaces the expected-frame field with that format's default: Auto=0, 35mm=6, 645=4, 6×6=3, 6×7=3, 6×8=2, 6×9=2. The existing 4×5 option keeps the current `defaultExpectedFrames` fallback of 6; this change does not define new 4×5 detection behavior.
...
2. Construct the property table with Auto selected and automatic frame count (`0`).
...
6. On the next invocation, repeat from Auto/0 regardless of the prior dialog's format.

## Verification

- Static contract test first fails against the current persisted-format code.
- Focused contract preloads stale 645/4 preferences and proves both recognition entries still initialize Auto index/0 without directly reading or writing the three removed preference keys.
...
- Rebuild and install the validated ZIP, restart Lightroom, select `52191.tif`, and confirm both recognition dialogs open as Auto/0 and automatically produce six-frame preview/copies.
- Reopen after selecting 645 in a prior run and confirm the next dialog again opens as Auto/0.

## Failure Handling

If Auto/0 causes a real fixture regression, stop at the failing evidence; do not compensate by changing detector heuristics. If the installed plugin does not match the rebuilt source hashes, reinstall from the extracted validated ZIP before repeating Lightroom acceptance.
