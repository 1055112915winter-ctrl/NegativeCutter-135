...
def _grayscale_2d(arr: np.ndarray) -> np.ndarray:
    """Normalize grayscale or multichannel image data to a 2D array."""
    arr = np.asarray(arr)
    if arr.ndim == 2:
        return arr
    if arr.ndim == 3:
...
def _bit_depth_from_array(arr: np.ndarray) -> int:
    arr = np.asarray(arr)
    if np.issubdtype(arr.dtype, np.integer) and arr.dtype.itemsize >= 2:
        return 16
    if arr.size and float(np.max(arr)) > 255.0:
        return 16
    return 8
...
def _load_dng_subifd(path: str) -> np.ndarray:
    ...
    with open(path, "rb") as fh:
        ...
        for row in range(height):
            ...
            row_arr = np.frombuffer(row_bytes, dtype=dtype).astype(np.uint16)
...
def load_dng_preview_array(path: str) -> tuple[np.ndarray, str, int]:
    """Load a DNG preview array for GUI display/export, preserving color when possible."""
    try:
        import rawpy  # type: ignore
        ...
        if arr.ndim == 3 and arr.shape[2] > 3:
            arr = arr[:, :, :3]
        return arr, "rawpy", _bit_depth_from_array(arr)
    except Exception:
        pass

    try:
        arr = _load_dng_subifd(path)
        return arr, "subifd", 16
    except Exception:
        pass
    ...
    if arr.ndim == 3 and arr.shape[2] > 3:
        arr = arr[:, :, :3]
    return arr, "pillow_fallback", _bit_depth_from_array(arr)
...
def estimate_rotation_with_diagnostics(
    arr, expected_frames, width, height, is_horizontal=True, mode="valley"
) -> RotationEstimate:
    """Estimate skew and reject measurements that are unsafe to apply."""
    if is_horizontal:
        if height is None or height < 10:
            return evaluate_rotation_offsets([], cross_baseline=0, cross_resolution=0)
        scan_size = width
        cross_size = height
    ...
    for f in range(1, expected_frames):
        ...
        for sub in subs:
            ...
            if mode == "peak":
                position = int(np.argmax(sub))
                prominence = float(sub[position] - median)
            else:
                position = int(np.argmin(sub))
                prominence = float(median - sub[position])
            centers.append(pc_min + position)
            local_prominences.append(max(0.0, prominence))
        if len(centers) == 2:
            offsets.append(centers[1] - centers[0])
            prominences.append(min(local_prominences))

    dy = (q3 - q1) / 2.0
    return evaluate_rotation_offsets(
        offsets,
        cross_baseline=dy,
        cross_resolution=cross_size,
        prominences=prominences,
    )


def estimate_rotation(arr, expected_frames, width, height, is_horizontal=True, mode="valley"):
    """Compatibility facade returning only the safe rotation angle."""
    return estimate_rotation_with_diagnostics(
        arr, expected_frames, width, height, is_horizontal, mode
    ).angle
...
_KNOWN_FORMAT_RATIOS = KNOWN_ASPECT_RATIOS
...
_MEDIUM_FORMAT_CODES = MEDIUM_FORMAT_CODES
...
def _should_use_120(
    ...
    """Compatibility name for the canonical medium-format route policy."""
    return should_use_medium_format(width, height, expected_frames, film_format)
...
def _detect_120_gaps(
    arr: np.ndarray,
    is_horizontal: bool,
    expected_frames_hint: Optional[int] = None,
):
    ...
    if expected_frames_hint is not None and int(expected_frames_hint) >= 1:
        frame_counts = (int(expected_frames_hint),)
    else:
        frame_counts = range(2, 7)
...
def analyze_image_120(
    ...
    # Detect on original
    orig_edges, orig_long_edges, n_frames = _detect_120_gaps(
        orig_arr,
        is_horizontal,
    ...
    debug_info = {
        ...
        "expectedFramesHint": expected_frames_hint,
        "longEdgeConfidence": {
...
def analyze_image(
    ...
    lr_width: Optional[int] = None,
    lr_height: Optional[int] = None,
    include_review_frames: bool = False,
    film_format: Optional[str] = None,
) -> dict:
    """
    ...
    include_review_frames : bool
        Include low-confidence frames in an error result so an interactive
        client can display them for manual correction. The default keeps the
        Lightroom safety behavior of withholding suspicious coordinates.
    ...
    # ------------------------------------------------------------------
    use_120, route_reason = _should_use_120(
        thumb_w,
        thumb_h,
        expected_frames,
        film_format,
    ...
    if use_120:
        return analyze_image_120(
            image_path,
            original_path,
            aspect_ratio,
            expected_frames_hint=expected_frames if expected_frames > 0 else None,
    ...
    rotation_estimate = estimate_rotation_with_diagnostics(
        orig_arr, expected_frames, orig_w, orig_h, is_horizontal, chosen_mode
    )
    crop_angle = rotation_estimate.angle
    ...
    debug_info["rotationEstimate"] = rotation_estimate.as_dict()
