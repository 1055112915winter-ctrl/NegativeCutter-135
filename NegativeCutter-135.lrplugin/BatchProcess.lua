  ...
  for index, photo in ipairs(photos) do
    ...
    if not detection then
      ...
      if settings.previewMode == "per_photo" then
        ...
        if preview.status == "error" then stats.errors[#stats.errors + 1] = detection.fileName .. ": " .. preview.error; stats.terminal = stats.terminal + 1; selected = nil
      ...
      elseif settings.previewMode == "batch_uniform" then
        if not sharedOffsets then
          ...
          if preview.status == "canceled" then stats.canceled = true; break end
          if preview.status == "error" then stats.errors[#stats.errors + 1] = detection.fileName .. ": " .. preview.error; stats.canceled = true; break end
          ...
          if not adjusted then stats.errors[#stats.errors + 1] = detection.fileName .. ": " .. tostring(adjustError); stats.terminal = stats.terminal + 1; selected = nil
