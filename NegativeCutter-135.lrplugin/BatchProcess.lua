...
local function selectedFormatHint(formatIndex) return FORMAT_OPTIONS[formatIndex] and FORMAT_OPTIONS[formatIndex].value or "" end

local function chooseSettings(photoCount)
  return LrFunctionContext.callWithContext("NegativeCutter.BatchProcess.chooseSettings", function(context)
    ...
    dialogData.previewModeIndex = optionIndex(PREVIEW_MODES, prefs.previewModeBatch or "batch_uniform")
    ...
    local settings = { expectedFrames = tonumber(dialogData.expectedFrames) or ProcessAgent.defaultExpectedFrames(selectedFormatHint(dialogData.formatIndex)),
      formatHint = selectedFormatHint(dialogData.formatIndex), filmType = filmTypes[dialogData.filmTypeIndex].value,
      previewMode = PREVIEW_MODES[dialogData.previewModeIndex].value }
    if settings.formatHint == "" then settings.formatHint = nil end
    prefs.filmType, prefs.previewModeBatch = settings.filmType, settings.previewMode
    return settings
  end)
end
