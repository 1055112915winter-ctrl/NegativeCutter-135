--[[
  NegativeCutter 导入代理 - 三种 Lightroom 集成模式
  1. XMP 边车导入
  2. HTTP API 调用
  3. JSON 文件监视
]]--
...
local logger = LrLogger('NegativeCutterImport')
...
local function importFromXMP()
  ...
  if not selectedPhotos or #selectedPhotos == 0 then
    LrDialogs.message("NegativeCutter - XMP 导入", "请先选择要导入的原始图像", "info")
    return
  end

  -- 目录选择器
  local dirResult = LrDialogs.runOpenPanel {
    title = "选择包含 NegativeCutter XMP 边车文件的文件夹",
  ...
  if #xmpFiles == 0 then
    LrDialogs.message("NegativeCutter - 未找到文件", "该文件夹中没有找到 .filmcrop.xmp 文件", "warning")
    return
  end
  ...
  if #xmpData == 0 then
    ...
    LrDialogs.message("NegativeCutter - 无匹配", msg, "warning")
    return
  end
  ...
  if #allErrors > 0 then
    ...
    msg = msg .. "\n\n错误:\n" .. errMsg
    LrDialogs.message("NegativeCutter - XMP 导入完成 (部分失败)", msg, "warning")
  else
    LrDialogs.message("NegativeCutter - XMP 导入完成", msg, "info")
  end
end
...
local function detectViaHttp()
  ...
  if not selectedPhotos or #selectedPhotos == 0 then
    LrDialogs.message("NegativeCutter - HTTP 检测", "请先选择要处理的胶片扫描文件", "info")
    return
  end
  ...
  local contents = f:column {
    spacing = f:control_spacing(),
    bind_to_object = dialogData,
    f:static_text { title = "NegativeCutter 独立引擎 API 地址:" },
  ...
  if #allErrors > 0 then
    ...
    LrDialogs.message("NegativeCutter - HTTP 检测完成 (部分失败)",
      string.format("创建虚拟副本: %d\n失败: %d\n\n%s", totalCreated, #allErrors, msg), "warning")
  else
    LrDialogs.message("NegativeCutter - HTTP 检测完成",
...
local function watchJsonFile()
  ...
  if prefs.watchActive and prefs.watchJsonPath then
    ...
    elseif result == "ok" then
      ...
      LrDialogs.message("NegativeCutter", "JSON 监视已停止", "info")
      return
    end
  ...
  if not selectedPhotos or #selectedPhotos == 0 then
    LrDialogs.message("NegativeCutter - JSON 监视", "请先选择要关联的原始图像", "info")
    return
  end

  local jsonPath = LrDialogs.runOpenPanel {
    title = "选择 NegativeCutter JSON 边车文件",
    canChooseFiles = true,
  ...
  if not ok then
    LrDialogs.message("NegativeCutter - JSON 导入失败", msg, "critical")
    return
  end

  LrDialogs.message("NegativeCutter - JSON 监视已启动",
    msg .. "\n\nNegativeCutter 将每 2 秒检查一次文件更新。\n（可通过「停止监视 NegativeCutter JSON」菜单项随时终止）",
    "info")
