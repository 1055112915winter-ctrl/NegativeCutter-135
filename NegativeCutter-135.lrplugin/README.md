...
两个识别入口都提供三种方式：**逐张预览**、**整批统一**、**不预览**。逐张预览会为每张成功识别的照片单独确认；整批统一只在第一张成功识别的照片上确认四个像素偏移，后续照片仍使用各自的检测结果和尺寸；不预览保持直接创建副本的流程。

预览对话框的上、下、左、右数值均表示向外扩展的像素。输入停止约 **120 毫秒** 后刷新，不会重新识别照片；渲染期间和失败时「**确认**」不可用。「**重置**」会把四项恢复为 0。处理期间 Lightroom 左上角显示原生**进度**，可随时**取消**；已成功创建的虚拟副本不会回滚。
...
3. 选择胶片格式；保持自动检测时会自动判断帧数，也可手动指定整批使用的帧数
4. 点击「开始批量处理」
...
### v2.4.7 (2026-07-29)
- **导出设置记忆**：独立版导出对话框可将当前格式、JPEG 质量和色彩空间保存为下次默认值
- **更顺手的重复导出**：下次打开导出对话框会自动带出上次保存的选择，输出目录仍按当前图像确定

### v2.4.6 (2026-07-17)
- **实时裁切预览**：识别后可在 Lightroom 中查看帧边界，并用四个像素偏移实时微调
- **批量预览模式**：支持逐张预览、整批统一和不预览，后续照片始终使用各自的检测结果与尺寸
- **原生进度与取消**：固定批次分母，取消后保留已成功创建的虚拟副本
- **运行时加固**：序列化预览请求、原子发布、last-good 保留和插件拥有目录的生命周期清理
- **结果分类修复**：零成功批次显示失败；首张失败后仍继续处理后续成功照片
- **120 预览方向修复**：分离缩略图预览坐标与 Lightroom 最终裁切坐标，避免 AD/CB TIFF 在预览中把上下排列的画格画成左右窄列
- **确认后再对齐**：逐张预览和整批统一均在用户确认后转换为 Lightroom 坐标，保留已验证正确的最终虚拟副本裁切
- **回归保护**：覆盖 AD 单张确认、批量统一像素偏移、清理顺序与预览/应用坐标不共享
