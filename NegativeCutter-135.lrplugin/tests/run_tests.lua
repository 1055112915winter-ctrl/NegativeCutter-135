...
runTest("orientation=CD 时应用 180° 旋转", function()
    local jsonStr = [[{
        "frameCount": 1, "sourceWidth": 3500, "sourceHeight": 2400,
        "cropAngle": 1.5,
        "frames": [{"index":1, "relativeTop":0.1, "relativeBottom":0.4,
    ...
    -- LR 照片是 horizontal，且 orientation=CD（180° 旋转）
    -- 宽高比一致，所以旧逻辑不会旋转；新逻辑通过 orientation 检测触发
    local photo = MockPhoto.createMockPhoto({
        dimensions = {width = 3500, height = 2400},
        rawMetadata = {orientation = "CD"}
    })

    local aligned = ProcessAgent.directionAlign(result, photo)

    -- 180° 旋转: top=1-0.4=0.6, bottom=1-0.1=0.9, left=1-0.8=0.2, right=1-0.2=0.8
    MockInit.assertEqual(0.6, aligned.frames[1].relativeTop, "180° top")
    MockInit.assertEqual(0.9, aligned.frames[1].relativeBottom, "180° bottom")
    MockInit.assertEqual(0.2, aligned.frames[1].relativeLeft, "180° left")
    MockInit.assertEqual(0.8, aligned.frames[1].relativeRight, "180° right")

    -- 180° 旋转不改变 cropAngle 符号
    MockInit.assertEqual(1.5, aligned.cropAngle, "cropAngle unchanged for 180°")
end)

runTest("orientation=AB 且宽高比一致时不旋转", function()
    local jsonStr = [[{
        "frameCount": 1, "sourceWidth": 3500, "sourceHeight": 2400,
        "frames": [{"index":1, "relativeTop":0.1, "relativeBottom":0.4,
                    "relativeLeft":0.0, "relativeRight":1.0}]
    ...
    MockInit.assertEqual(0.1, aligned.frames[1].relativeTop, "top unchanged")
    MockInit.assertEqual(0.4, aligned.frames[1].relativeBottom, "bottom unchanged")
    MockInit.assertEqual(0.0, aligned.frames[1].relativeLeft, "left unchanged")
    MockInit.assertEqual(1.0, aligned.frames[1].relativeRight, "right unchanged")
end)
