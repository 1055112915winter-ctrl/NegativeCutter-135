...
- **Use `json.decode` instead of regex for JSON parsing:** The plugin bundles its own pure-Lua decoder at `json.lua` in the plugin root. Lightroom SDK does NOT register a `json` toolkit script — `require("json")` fails with "Could not load toolkit script: json" (LR ships `JSON.lua` uppercase with method-call API, but the lowercase name is not registered). Plugin code loads the bundle via `dofile(LrPathUtils.child(pluginPath, "json.lua"))` at module top, the same pattern used for ProcessAgent.lua / ApplierAgent.lua. Stub also lives at `tests/mock_sdk/json.lua` for vanilla-lua test runs.
- **`pcall` cannot wrap yielding LR SDK calls:** `pcall(function() return photo:getFormattedMetadata(...) end)` and similar wrappers raise "Yielding is not allowed within a C or metamethod call" because pcall is a C boundary that disallows coroutine yields. Inside an `LrTasks.startAsyncTask` block, call yielding methods (`getFormattedMetadata`, `getRawMetadata` for some keys, `catalog:withWriteAccessDo`, etc.) directly — the outer task's pcall is sufficient for error capture, and the async task itself is yield-safe. Apply pcall only to non-yielding calls (numeric/string utilities, `LrPathUtils.*`, file IO via `LrFileUtils.*`).
