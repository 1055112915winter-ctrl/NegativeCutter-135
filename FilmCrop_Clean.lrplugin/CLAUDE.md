...
- **`pcall` cannot wrap yielding LR SDK calls; use `LrTasks.pcall` inside async tasks:** Lua's built-in `pcall` is a C boundary that disallows coroutine yields, so wrapping a yielding SDK call (`photo:getFormattedMetadata`, `catalog:getTargetPhotos`, `catalog:withWriteAccessDo`, etc.) raises "Yielding is not allowed within a C or metamethod call". Inside `LrTasks.startAsyncTask`, use **`LrTasks.pcall(func, ...)`** instead — same signature as `pcall` but yield-safe. Reserve plain `pcall` for purely non-yielding calls (string/numeric utilities, `LrPathUtils.*`, `LrFileUtils.*`).
