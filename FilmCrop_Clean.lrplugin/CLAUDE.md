...
- **Use `json.decode` instead of regex for JSON parsing:** The plugin bundles its own pure-Lua decoder at `json.lua` in the plugin root (Lightroom SDK does NOT provide `require("json")` — it ships `JSON.lua` with method-call API which is incompatible). Prefer the bundled `require("json")` over fragile Lua regex patterns for parsing `.filmcrop.json` sidecars. Stub also lives at `tests/mock_sdk/json.lua` for vanilla-lua test runs.
